...
# ---------------------------------------------------------------------------
# Cost / timeline model (adapted from compl_ai roadmap_generator architecture)
# ---------------------------------------------------------------------------

class DevelopmentPhase(BaseModel):
    name: str               # e.g. "Phase II Clinical Trial"
    cost_low_usd: int       # lower bound (out-of-pocket, 2024 USD)
    cost_high_usd: int      # upper bound
    years_low: float        # lower bound on duration (after designation adjustments)
    years_high: float       # upper bound
    notes: Optional[str] = None


class DevelopmentCostEstimate(BaseModel):
    pathway: str                     # e.g. "505(b)(1)"
    phases: list[DevelopmentPhase]   # per-phase breakdown

    # Aggregated point-estimate ranges (sum of phase bounds)
    total_cost_low_usd: int
    total_cost_high_usd: int
    total_years_low: float
    total_years_high: float

    # Monte Carlo percentiles on total cost and total years
    cost_p10_usd: int
    cost_p50_usd: int
    cost_p90_usd: int
    years_p10: float
    ...
    designations_applied: list[str]  # which designations influenced the estimate
    summary: str                     # plain-English summary paragraph


# ---------------------------------------------------------------------------
# Patent radar (adapted from compl_ai IP radar — USPTO PatentsView API)
# ---------------------------------------------------------------------------
...
class DrugPatentResult(BaseModel):
    patent_number: str
    title: str
    abstract: str
    assignee: str
    ...
    relevance_explanation: str          # plain-English LLM explanation
    concerning_claims: list[str] = []   # specific claim language of concern


class PatentRadarResult(BaseModel):
    patents: list[DrugPatentResult]
    search_queries_used: list[str]
    red_count: int
    yellow_count: int
    summary: str   # plain-English LLM landscape summary
    disclaimer: str = (
        "This output is generated by an AI system and does not constitute legal advice "
        "or a Freedom-to-Operate opinion. Consult a registered patent attorney before "
        "making IP-related business decisions."
    )
...
class TargetIQResponse(BaseModel):
    ...
    cost_estimate: Optional[DevelopmentCostEstimate] = None  # phase-by-phase cost/timeline
    patent_radar: Optional[PatentRadarResult] = None         # IP landscape for the target
